...
const MAP = {
  // x/y is the LOWEST-X, LOWEST-Y corner of the 3 x 3 trap footprint.
  // Bear 1 centre 482,587 -> x=481 y=586.  Bear 2 centre 473,589 -> x=472 y=588.
  bears: [
...
function validate(assigned, map) {
  ...
  const items = [
    ...
    ...map.bears.map((b) => ({ kind: 'bear', name: b.name, x: b.x, y: b.y, w: b.w || 3, h: b.h || 3 })),
  ...
  for (const a of assigned) {
    ...
    if (!isDual(a.player) && rectsTouchOrOverlap(a.spot, { x: b1.x, y: b1.y, w: b1.w || 3, h: b1.h || 3 })) {
...
module.exports = {
  ...
  isDual,
  rectsOverlap,
  bearFromRing: require('./map').bearFromRing,
  bearFromCentre: require('./map').bearFromCentre,
  objectFromCoordinate: require('./map').objectFromCoordinate,
  lintMap: require('./map').lintMap,
};
